...
class AttendanceDevice(models.Model):
    ...
    _sql_constraints = [
        ...
        ('float_hour_less_than_24_check',
         'CHECK(auto_clear_attendance_hour < 24.0)',
         "Auto Clear At must be less than 24"),
        ('float_hour_greater_than_or_equal_to_zero',
         'CHECK(auto_clear_attendance_hour >= 0)',
         "Auto Clear At must be greater than or equal to zero (0.0)"),
    ]
                ...
                if user_id:
                    ...
                    if not duplicate_attend:
                        try:
                            ...
                            error_msg += str(e) + "<br />"
                            error_msg += _("Error create DeviceUserAttendance record: device_id %s; user_id %s; timestamp %s; attendance_state_id %s.<br />") % (r.id, user_id.id, timestamp_str, attendance_states[attendance.punch])
