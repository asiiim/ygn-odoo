...
class ProductProduct(models.Model):
    ...
    @api.onchange('weight', 'loss', 'wage', 'gems_cost', 'base_product_id')
    def compute_sales_price(self):
        for product in self:
            if product.can_depend_base:
                product.fix_price = ((product.weight + product.loss) * product.base_product_id.list_price) + product.wage + product.gems_cost
